...
const CollectionVideo: FC<IProps> = (props) => {
  ...
  return <CollectionVideoWrapper>
    <div className="col-header">
      <span className="name">{collection && collection.name}合集</span>
      <span className="count">({currentIndex+1 }/{ videoList.length})</span>
    </div>
    <ul className="video-list">
      {
        videoList && videoList.length !== 0 && videoList.map((item, index) => {
          return <li key={item.id} className="item" onClick={() => videoClick(item, index)}>
            {
              currentIndex===index&&<PauseOutlined className="gb-play" spin={false} style={{color:index===currentIndex?'':'#f1f2f3'}}/>
            }
            {
              currentIndex !== index && <span className="sort">{ (index+1).toString().padStart(2,"0")}</span>
            }
