...
const VideoList:FC<IProps>=(props)=>{
  ...
  return (
      <VideoListWrapper>
            ...
            [0,1].map((item,index)=>{
              return (
                  <ul key={item} className="common-video-list">
                    ...
                    {
                      videoList && (videoList.length> 5) && <Fragment>
                        <div className={'prev'} onClick={e=>prevHandle()}> <LeftOutlined/></div>
                        <div className={'next'} onClick={e=>nextHandle()}><RightOutlined/> </div>
                      </Fragment>
                    }
                  </ul>
              )
        ...
        {
          (!videoList || videoList.length===0)&& <Empty description={"暂无视频"} image={Empty.PRESENTED_IMAGE_SIMPLE} />
        }
      </VideoListWrapper>
  )
}
