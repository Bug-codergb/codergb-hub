...
const VideoList:FC<IProps>=forwardRef<IVideoListRef,IProps>((props,propsRef)=>{
  ...
  const getVideoList = async (offset:number,limit:number,keyword:string,isSelf:boolean)=>{
    if(isSelf){
      const result = await getUserVideo<IResponseType<IPage<IVideo[]>>>(loginMsg.userMsg.userId,keyword,offset,limit);
        ...
        <ul className="vio-list">
          {
            videoList && videoList.length!==0&& videoList.map((item,index)=>{
              return (
                  <li key={item.id} onClick={e=>selectVideo(item,index)} className={currentIndex===index?'active':''}>
