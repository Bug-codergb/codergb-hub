import { uploadVideo } from '@/network/video';
...
async function chunkHandle(
  ...
  try {
    ...
    if (uploadedSize > total) {
      ...
      const websocket = new WebSocket(`${WEBSOCKET_HOST_NAME}/video/merge${params}`); //发送合并请求
    ...
    const blobFile = file.slice(index * chunkSize, end);

    const formData = new FormData();
    const fileBlob = new File([blobFile], name, {
      type: type
    });
